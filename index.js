    ...
    fetch(searchLink, {
      ...
      .then((result) => {
        ...
        for (let i = 0; i < result.common.length; i++) {
          ...
          link.innerHTML =
            result.common[i].food_name +
            ' ' +
            '<span id="grey">' +
            result.common[i].serving_qty +
            ' ' +
            result.common[i].serving_unit +
            '</span>' +
            '<img src="' +
            result.common[i].photo.thumb +
            '">' +
            '<span class="searchId hide">' +
            'name' +
            '</span>'
        }
        ...
        for (let i = 0; i < result.branded.length; i++) {
          ...
          link.innerHTML =
            result.branded[i].food_name +
            '<span class="brandName">' +
            ' ' +
            result.branded[i].brand_name +
            '</span>' +
            ' ' +
            '<span id="grey">' +
            result.branded[i].serving_qty +
            ' ' +
            result.branded[i].serving_unit +
            '</span>' +
            '<img src="' +
            result.branded[i].photo.thumb +
            '">' +
            '<span class="searchId hide">' +
            result.branded[i].nix_item_id +
            '</span>'
        }
        ...
        for (let i = 0; i < searchResultChilds.length; i++) {
          searchResultChilds[i].addEventListener('click', (event) => {
            if (event.originalTarget.classList.contains('searchLinks')) {
              ...
              fetch(searchLinkSecond, {
                ...
                .then((result) => {
                  ...
                  document.querySelector('#infoWeigthName').textContent =
                    result.foods[0].food_name

                  //primary quantity in select
                  let optionPrimary = document.createElement('option')
                  document
                    .querySelector('#infoWeightSelect')
                    .appendChild(optionPrimary)
                  ...
                  if (result.foods[0].alt_measures !== null) {
                    for (
                      let i = 0;
                      i < result.foods[0].alt_measures.length;
                      i++
                    ) {
                      if (
                        result.foods[0].alt_measures[i].measure !==
                        document.querySelector('#infoWeightSelect').firstChild
                          .innerHTML
                      ) {
                  ...
                  document.querySelector('#infoWeightInput').value =
                    result.foods[0].serving_qty

                  protein = result.foods[0].nf_protein
                  document.querySelector('#infoWeigthP').textContent =
                    protein + 'g'

                  carbs = result.foods[0].nf_total_carbohydrate
                  document.querySelector('#infoWeigthC').textContent =
                    carbs + 'g'

                  fat = result.foods[0].nf_total_fat
                  document.querySelector('#infoWeigthF').textContent = fat + 'g'

                  document.querySelector('#infoWeigthK').textContent =
                    result.foods[0].nf_calories + 'kcal'
                  console.log(result.foods[0])

                  document.querySelector('#cholesterolAmount').textContent =
                    result.foods[0].nf_cholesterol + 'mg'

                  document.querySelector('#saturatedFatAmount').textContent =
                    result.foods[0].nf_saturated_fat + 'g'

                  document.querySelector('#sodiumAmount').textContent =
                    result.foods[0].nf_sodium + 'mg'

                  document.querySelector('#sugarsAmount').textContent =
                    result.foods[0].nf_sugars + 'g'

                  document.querySelector('#potassiumAmount').textContent =
                    result.foods[0].nf_potassium + 'mg'

                  document.querySelector('#fiberAmount').textContent =
                    result.foods[0].nf_dietary_fiber + 'g'

                  // LOOP FOR ALL THE OTHER NUTRIENTS
                  for (
                    let n = 0;
                    n < result.foods[0].full_nutrients.length;
                    n++
                  ) {
                    for (let s = 0; s < nutrients.length; s++) {
                      if (
                        result.foods[0].full_nutrients[n].attr_id ===
                        nutrients[s].attr_id
                      ) {
                    ...
                    tableData2.textContent =
                      result.foods[0].full_nutrients[n].value +
                      ' ' +
                      nutrient.unit
...
window.setInterval(function () {
  document.querySelector('#random').style.color =
